...
def capture_posture_score():
    ...
    print("Camera ready. Press 'S' to start recording and 'Q' to stop.")
    ...
    # Process video until Q is pressed
    while True:
        ret, frame = cap.read()
        if not ret:
        ...
        if not recording:
            cv2.putText(frame, "Press 'S' to start recording", (10, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 1)
        else:
            # Calculate elapsed time
            elapsed_time = time.time() - start_time
            cv2.putText(frame, f"Recording: {elapsed_time:.1f}s (Press 'Q' to stop)", 
            ...
            cv2.putText(frame, f"Score: {productivity_score:.1f}", (10, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 1)
            cv2.putText(frame, presence_text, (10, 90), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 1)
    ...
    # Calculate statistics if recording was done
    if recording and productivity_scores:
        ...
        print("No recording was made or no valid frames were processed.")
        return None
...
def preprocess_data(path):
    ...
    df['experience'] = (datetime.today() - df['Joining Date']).dt.days / 365.0
    ...
    # Projects completed per year of experience (adjusted for diminishing returns)
    df['efficiency_index'] = df['projects_completed'] / np.log1p(df['experience'])
    # Performance normalized by age expectations
    df['age_adjusted_performance'] = df['projects_completed'] * (df['age'] / df['experience'])
    return df

def train_models(df):
    X = df[['age', 'experience', 'projects_completed', 'posture_score', 'efficiency_index', 'age_adjusted_performance']]
    y = df['output']
...
def predict_employee_performance(efficiency_index, age_adjusted_performance, posture_score, age=57, experience=26, projects_completed=23, models=None):        
    lr_model, rf_model, xgb_model, scaler = models
    
    feature_names = ['age', 'experience', 'projects_completed', 'posture_score', 'efficiency_index', 'age_adjusted_performance']
    input_df = pd.DataFrame([[age, experience, projects_completed, posture_score, efficiency_index, age_adjusted_performance]], 
...
def plot_performance_distribution(df):
    ...
    ax = sns.countplot(x='output', hue='output', data=df, palette='viridis', legend=False)
...
if __name__ == '__main__':
    ...
    print("\nStarting employee productivity analysis based on posture...")
    posture_results = capture_posture_score()
    ...
    if posture_results:
        ...
        _, performance_category = predict_employee_performance(
            efficiency_index=employee_projects/np.log1p(employee_experience),
            age_adjusted_performance=employee_projects*(employee_age/employee_experience),
